...
- [x] U2/A2：24 张同视角建筑状态图已在批次⑤交付、导入并通过园区密集态回归。
...
- [x] U1/U2/U3：建筑统一增加地块基座，园区列距 +24u、行距 +32u；世界告警增加 1↔1.08 呼吸和 2u 白描边。A2 交付前使用 0.18 程序化投影，交付后由批次⑤的素材内落影替代。
...
### 批次⑤ · A2 建筑全状态素材重做与接入（2026-08-03）

- [x] 先建立 T0–T3 四张 active 母版，再从各自母版派生 `construction / dark / aged / decayed / ruin`；24 张统一为左下朝向 30° 俯视、蓝/米白材质、同厚度混凝土地基与同方向落影。
- [x] 四级轮廓重新分层：T0 集装箱机房、T1 单层三冷却单元、T2 双层四台屋顶 AC、T3 双塔超大规模园区；在 440×956 密集六地块中仍可一眼区分。
- [x] 使用边界自动取样的纯绿色键工作流去背；24 张 alpha 外边界均为 0，且 `alpha > 32` 区域无绿色键色残留。
- [x] 每个 tier 以六态联合 alpha 包围盒共用同一缩放/居中变换，避免施工吊车贴边，以及运行状态切换时建筑跳位或忽大忽小；成品严格为 T0/T1 768²、T2 1024²、T3 1280²。
- [x] 正式资源写入 `art-renders/visual/final/buildings/dc_t*.png` 并通过 import 管线同步至 `assets/art/buildings/`；T3 使用高质量 PNG 调色板压缩满足 manifest 单文件上限。
- [x] 移除 A2 自带落影上方的旧程序化椭圆影；保留地块基座。`map_built` 视觉快照改为等待建造完成弹跳结束后采集，保证审阅稳定成品帧。
- [x] 资产联系表：`docs/ui_review/09_a2_active_tiers_contact.png`、`docs/ui_review/09_a2_buildings_contact.png`；实机布局联系表：`docs/ui_review/09_a2_runtime_contact.png`。

门禁：`validate_data.py` 通过；`check_assets.py --strict` 134/134；`test_runner` 103/103；`visual_smoke` zh_CN 30/30、en 30/30。
