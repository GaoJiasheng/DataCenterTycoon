...
| 表现 | `ui/main_view.gd`、`gameplay/map/park_map.gd` | 竖屏六导航入口、地图拖拽缩放、正式资源、详情、弹窗、引导和事件特效 |
...
当前 `save_version=1`，主档为 `user://save_v1.json`。核心字段包括：

- `player`：现金、钻石、总收入、时代、网络、品牌倍率和累计建设数；
- `plots`：地块与嵌套机房，机房包含 9 个机柜格、供电、四边冷却、客户、寿命与状态；
- `construction_queue`：最多两个持久化工程及广告加速次数；
...
游戏月为 7200 现实秒、游戏年为 86400 现实秒。`Game.advance_time()` 在建设、维修、故障、维护和行情边界处分段积分，不逐秒遍历；测试覆盖 90 个现实日的一次性快进。
...
桥接单例名固定为 `DataCenterAdsBridge`。只有 SDK 的用户实际获得奖励回调可以发送 `earned=true`。四个 placement 前缀为 `offline_double`、`repair:`、`construction:`、`arrears_rescue`；`noads` 权益保留相同频控但直接发奖。
