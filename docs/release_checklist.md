...
- [x] `python3 tools/validate_data.py`（2026-08-31：18 表、双语、180 art IDs、归属与许可证路径）
- [x] `godot --headless --path . tests/test_runner.tscn`（2026-08-31：249 passed）
- [x] `flow_audit` / `midgame_audit` / 真实触摸 `tutorial_playthrough` / `full_campaign`（2026-08-31：全部通过；本轮首局第 111 月达 20 座、完成重组，二局第 11 月达 21 座并续跑至第 30 月 59 座）
- [x] `visual_smoke` 中英标准档各 51 态（990×2151，含 `legal_view` / `attributions_view`），SE / 大画幅中英各 8 个关键态（750×1334 / 1024×1366，后者保留 `--profile=ipad` 接口），2026-08-31 全量复跑通过裁剪、贴边、叠印与触控断言
- [x] `godot --disable-vsync --max-fps 240 --path . tests/performance_smoke.tscn`（2026-08-31：100 座机房、13 园区页、当前页 6 对象；average 7.82ms / p90 10.51ms / p95 10.69ms / 30 粒子与猫特效零残留 / 节点差 0）
...
- [x] `python3 tools/check_assets.py --strict --audio`（2026-08-31：180 art / 6 font / 23 audio，纹理分类 45 无损 + 135 VRAM；两份 OFL 与字体成对）
- [x] `python3 tools/check_provenance.py`（2026-08-31：台账精确覆盖 art=180 / audio=23 / font=6，未知来源与授权均保留“待确认”）
- [x] `python3 tools/check_app_store_assets.py`（2026-08-30：1.0 iPhone-only 契约，中英 × iPhone 6.9 × 五屏，共 10 张原生离屏渲染截图；尺寸、组内一致性与无 alpha 全绿）
- [ ] `python3 tools/check_release.py`（2026-08-31：PCK 内三份法务正文与两份 OFL、翻译和商店素材均通过；按设计只阻断 7 个 `release_identity.json` 所有者占位值与 IAP 插件描述符）
...
- [ ] iPhone SE-class, 6.7-inch iPhone and notched/Dynamic Island iPhone smoke tests
- [ ] 可选且不阻断发布：在 iPad 的 iPhone 兼容模式抽查启动、触控与退出
...
- [x] Five ordered screenshots for 6.9-inch iPhone in both languages; see `docs/store/README.md`
