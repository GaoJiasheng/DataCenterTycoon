...
- [x] 新增 `tests/store_shots.tscn` / `store_shots.gd`，真实 `main.tscn` 挂在目标分辨率 `SubViewport` 中原生渲染；实际运行不依赖桌面窗口高度，不存在小图放大。
- [x] `tests/store_showcase_fixture.gd` 只位于测试目录；中英使用同一份后期存档、同一相机和同一页面状态，只切换本地化。园区、机房、行情、科技与上市重组五屏均由权威游戏数据驱动，未伪造一套截图 UI。
- [x] 最终审片发现行情数据虽完整、首稿却把曲线留在可视区之下；管线现在只滚动真实 `PageScroll` 到两张询价人设卡与 `MarketChart` 的交界，同屏能看到两位客户与历史曲线。夹具预先标记已完成成就，避免截图途中成就奖励使 HUD 钻石在五屏间变动；四种组合已整套重拍通过。
- [x] 交付 [20 张审片索引](store/screenshots/README_review.md)：中英 × iPhone 6.9英寸 `1320×2868` / iPad 13英寸 `2048×2732` × 五屏；全部为无 alpha PNG。iPad 保留现行 `aspect=keep`，信箱填充色是审定的 `#122438`。
- [x] `python3 tools/check_app_store_assets.py`：图片总数、尺寸、组内一致性、不透明图标与截图全部通过。README 已列出四种组合的可重放命令。

### G2 · 发布身份与 G3 · 商店文案

- [x] 发布身份收敛到 `data/release_identity.json`；产品名、两个邮箱、两个 URL、生效日期与广告商全部保留 `REPLACE_WITH_*` 所有者占位，没有伪造任何身份值。
- [x] `tools/fill_release_identity.py` 从四份 `.tmpl` 一次生成隐私页、支持页与中英 metadata；`check_release.py` 会先拦占位值，再做模板/产物逐字一致校验，手改产物无法静默混过。
- [x] 中英文案已补齐推广文本、完整描述、首版更新说明、审核备注、四个激励视频入口、恢复购买路径、八个 SKU 沙盒步骤与年龄分级草案；长度门禁通过，产品名仍留给所有者。
- [x] `python3 tools/check_release.py` 的实际预期结果已得到：翻译、17 张数据表、180 个美术 ID、图标与 20 张截图都通过；只阻断上述 7 组所有者占位值与缺少 iOS StoreKit/IAP `.gdip` 描述符。**红灯是本批次要求的正确结果。**

### G4 · Toast 闪测

- [x] 在后台持续运行标准英文视觉渲染的负载下，独立连跑 20 轮 `test_runner`，20/20 均为 `243 passed, 0 failed`；目标拒绝操作断言 20/20 通过。
- [x] 本轮未出现 stage C/D 的可见性或文本诊断失配，因此按规格如实记录"未复现"；未改产品 toast 优先级、未改测试夹具、未放宽复合断言。详细证据见 [24 G4 高负载记录](test_runs/24_g4_toast_stress.md)。

### G5 · 二周目覆盖

- [x] `full_campaign` 在第一次上市重组后再跑 30 个游戏月；本轮首局第 119 月达 20 座，二局第 11 月达 21 座、第 30 月 59 座，真实兑现"财富转速度"。
- [x] 转生后工程部先回到 2 队列，再用带回现金按正式价格重购 L2 恢复 3 队列，并一次容纳 3 个真实 T1 建设项目。
- [x] 董事会点数可分配，商业加成与 `Rules.datacenter_income_per_month` 权威公式逐值对账；公司纪念册恰好增加一条，路线图/典藏保留且重复领取不发钻石。
- [x] 二局 30 月每个分段都断言无负现金、无 `game_over`，尾声重跑五页导航与机房抽屉；完整门禁本轮 81.96 秒。历史基线没有保存可比秒数，因此不伪造增幅。
- [x] 将"二局比首局更快"的断言故意取反后，门禁会稳定报出 1 个对应错误；恢复后重跑全绿，证明新覆盖不是空断言。详细数据同步到 [17 号全流程审计](17_full_campaign_audit.md)。

### 全量门禁与零漂移

- [x] `validate_data`：17 张数据表、双语本地化和 180 个美术 ID 通过；`test_runner` 243/243，`flow_audit`、`midgame_audit`、真实触摸 `tutorial_playthrough`、二周目 `full_campaign` 全绿。
- [x] 标准画幅中英各 49 态、SE 中英各 8 态、iPad 中英各 8 态全绿；小屏抽屉底部 CTA、教程聚光、人设卡、离线日志与 iPad 信箱无裁切回归。
- [x] `check_assets --strict --audio`：180/180 美术、6/6 字体、23/23 音频，纹理分类为 45 张无损 UI + 135 张 VRAM 场景图；`check_app_store_assets` 全绿。
- [x] 百机房性能门禁在外部高负载退出后独占复测通过：13 个园区页、当前页 6 对象、统一朝向与单猫状态机正常，average `7.67ms`、p90 `12.40ms`、p95 `15.75ms`；30 粒子、猫爱心与节点增量均归零。首轮受十余个外部 Godot 满载进程干扰的失败样本未被冒充为验收证据。
- [x] `report_release_economy.py`：T2 六档 × 20 种子、工程部 L4、钻石三策略 × 三时代 × 20 种子全部通过，未落任何正式数值。
- [x] `simulate_economy.py` 输出 SHA-256 仍为 `4e8d551354da4bff23232b454abb9253f2ae96e5a3f8c03b60b8fe732a227e04`，与 `720072b` 基线逐字一致；20 座中位 19.3 天、活跃/挂机净值比 23.67×，本批次零玩法、零经济、零随机漂移。
