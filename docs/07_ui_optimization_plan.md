...
| WP7 双语视觉/性能/文档门禁 | ✅ 2026-08-02（仓库内） | `visual_smoke --locale=en|zh_CN` 各跑 30 态，统一检查 glyph、`×0.00`、滚动保持、44pt 触控、文字边界、棋盘/行情/高光结构；新增六机房 + 30 金币 Metal 压测，p95 必须低于 16.67ms 且粒子/节点零泄漏；`ios_ui_redesign`、`architecture`、README 与 release checklist 已同步。iPhone 12/17 的 Instruments 定量复核仍是必须由真实设备完成的发行门禁，不以桌面结果冒充。 |
...
每步完成后重跑双语 30 态 visual_smoke 并对照 9.2 表逐项打钩；最终以核心测试、双语视觉门禁、Metal 性能烟测与外部发行审计四组结果交付。
