...
| WP5 FTUE 聚光灯 | ❌ | 仍白色横幅 |
| WP6 时代演出 / 商店分区 / 设置合规项 | ❌ | 均原样 |

### 9.3 首轮引入的新问题（随下一轮一并修）

1. **术语暴露**：「100% 行情敏感」「30% 行情敏感」是设计参数不是玩家语言。改为图标 + 「随行情波动 / 收益稳定」两档文案（新 key `RACK_TRAIT_VOLATILE` / `RACK_TRAIT_STABLE`），敏感度数值只进长按 tooltip。
2. **机柜选购信息不足**：选项仅名称+价格+敏感度，缺 耗电/发热/基础产出 三项关键数据（WP3-2 的验收项，提前到下一轮）。
